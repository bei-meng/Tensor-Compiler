...
def generate_matmul_test(name, input_dim, weight_dims):
    ...
    print(f"生成 onnx_files/{name}.onnx")
...
generate_matmul_test("const_add_relu_matmul_2x1", 2, [2, 1])

# Square matrix: x[2] -> expand[1,2] @ weight[2,2] -> [2]
generate_matmul_test("const_add_relu_matmul_2x2", 2, [2, 2])

# Rectangular: x[3] -> expand[1,3] @ weight[3,2] -> [2]
generate_matmul_test("const_add_relu_matmul_3x2", 3, [3, 2])

# Large Square: x[10] -> expand[1,10] @ weight[10,10] -> [10]
generate_matmul_test("const_add_relu_matmul_10x10", 10, [10, 10])