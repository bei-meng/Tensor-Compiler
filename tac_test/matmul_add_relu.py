...
def create_fused_matmul(dim):
    ...
    filename = f'onnx_files/matmul_add_relu{dim}.onnx'
    onnx.save(model, filename)
    print(f"生成 onnx_files/matmul_add_relu{dim}.onnx")
    with open(f"proto_files/matmul_add_relu{dim}.onnx", "w") as f:
        f.write(str(model))

create_fused_matmul(512)
create_fused_matmul(1024)
create_fused_matmul(2048)
