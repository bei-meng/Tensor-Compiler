    ...
    %2 = scf.for %arg3 = %c0 to %c2048 step %c64 iter_args(%arg4 = %1) -> (tensor<2048x2048xf32>) {
      %5 = scf.for %arg5 = %c0 to %c2048 step %c64 iter_args(%arg6 = %arg4) -> (tensor<2048x2048xf32>) {
        %6 = scf.for %arg7 = %c0 to %c2048 step %c64 iter_args(%arg8 = %arg6) -> (tensor<2048x2048xf32>) {
          %extracted_slice = tensor.extract_slice %arg0[%arg3, %arg5] [64, 64] [1, 1] : tensor<2048x2048xf32> to tensor<64x64xf32> loc(#loc1)
          %extracted_slice_0 = tensor.extract_slice %arg1[%arg5, %arg7] [64, 64] [1, 1] : tensor<2048x2048xf32> to tensor<64x64xf32> loc(#loc1)
          %extracted_slice_1 = tensor.extract_slice %arg8[%arg3, %arg7] [64, 64] [1, 1] : tensor<2048x2048xf32> to tensor<64x64xf32> loc(#loc1)
          %7 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "reduction", "parallel"]} ins(%extracted_slice, %extracted_slice_0 : tensor<64x64xf32>, tensor<64x64xf32>) outs(%extracted_slice_1 : tensor<64x64xf32>) {
          ^bb0(%in: f32 loc(unknown), %in_2: f32 loc(unknown), %out: f32 loc("matmul_out")):
            %8 = arith.mulf %in, %in_2 : f32 loc(#loc1)
            %9 = arith.addf %out, %8 : f32 loc(#loc1)
            linalg.yield %9 : f32 loc(#loc1)
          } -> tensor<64x64xf32> loc(#loc1)
          %inserted_slice = tensor.insert_slice %7 into %arg8[%arg3, %arg7] [64, 64] [1, 1] : tensor<64x64xf32> into tensor<2048x2048xf32> loc(#loc1)
